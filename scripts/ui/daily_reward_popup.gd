...
const STREAK_DOT_FILLED: String = "●"
const STREAK_DOT_EMPTY: String = "○"
...
@onready var _streak: Label = $Center/Panel/VBox/Streak
...
func show_reward(result: Dictionary) -> void:
	...
	_streak.text = "%d günlük seri\n%s" % [streak, _streak_dots(streak)]
...
func _streak_dots(streak: int) -> String:
	...
	var dots: String = STREAK_DOT_FILLED.repeat(filled) \
		+ STREAK_DOT_EMPTY.repeat(STREAK_DOTS - filled)
